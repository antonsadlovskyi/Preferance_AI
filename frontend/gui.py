...
class GUI:
    ...
    def draw_players(self):
        ...
        Label(frame,text='ВИ',fg='#78b1e8' if self.games[self.current_game].hands[SOUTH].highlight else 'white',bg=BACKGROUND_COLOR).place(relx=0.5,rely=0.86,anchor='center')
        ...
        for i,card in enumerate(sorted(self.games[self.current_game].hands[WEST].cards)):
            ...
            if not self.games[self.current_game].hands[WEST].highlight or card.highlight=='disabled':
                button["state"]='disabled'
        Label(frame,text='АНТОН',fg='#78b1e8' if self.games[self.current_game].hands[WEST].highlight else 'white',bg=BACKGROUND_COLOR).place(relx=0.5,rely=0.975,anchor='center')
        frame=tk.Frame(self.root,bg=BACKGROUND_COLOR,bd=0,height=9*CARD_STEP_Y+CARD_Y_SIZE+2*BORDER_THICKNESS+0.3*CARD_Y_SIZE,width=CARD_X_SIZE+2*BORDER_THICKNESS)
        ...
        for i,card in enumerate(sorted(self.games[self.current_game].hands[EAST].cards)):
            ...
            if not self.games[self.current_game].hands[EAST].highlight or card.highlight=='disabled':
                button["state"]='disabled'
        Label(frame,text='ЮЛЯ',fg='#78b1e8' if self.games[self.current_game].hands[EAST].highlight else 'white',bg=BACKGROUND_COLOR).place(relx=0.5,rely=0.975,anchor='center')
    ...
    def show_info(self,*args):
        for widget in self.root.winfo_children():
            if '!toplevel' in str(widget):
                return
        contract_interpreter={'M':'MISERE','P':'ГРАТИ'}
        player_interpreter={0:'ВИ',1:'АНТОН',2:'ЮЛЯ'}
        # suits_interpreter = {0: 'SPADES', 1: 'DIAMONDS', 2: 'CLUBS', 3: 'HEARTS', '-': 'NONE'}
        suits_interpreter={0:'ПІКИ',1:'БУБНА',2:'ХРЕСТА',3:'ЧЕРВИ','-':'НЕМАЄ'}
        info_window=tk.Toplevel(self.root)
        info_window.geometry(f"{int(X_SIZE//2.7)}x{X_SIZE//5}")
        info_window.title("Інфо")
        frame=tk.Frame(info_window,bd=0,bg='pink3')
        frame.pack(fill='both',side=tk.TOP,expand=True)
        Label(frame,anchor='w',text=f' ТИП КОНТАКТУ:',bg='#475c49',fg='white').pack(fill='both',side=tk.LEFT,expand=True,ipady=MESSAGE_FONT//6)
        Label(frame,anchor='e',text=f'{contract_interpreter[self.games[self.current_game].params["type"]]} ',bg='#475c49',font=tkf.Font(family="Garamond",size=MESSAGE_FONT),fg='white').pack(fill='both',side=tk.RIGHT,expand=True,ipady=MESSAGE_FONT//6)
        frame=tk.Frame(info_window,bd=0,bg='white')
        frame.pack(fill='both',side=tk.TOP,expand=True)
        Label(frame,anchor='w',text=f' ГРАЮЧИЙ РУКА:',bg='white',fg='#475c49').pack(fill='both',side=tk.LEFT,expand=True,ipady=MESSAGE_FONT//6)
        Label(frame,anchor='e',text=f'{player_interpreter[self.games[self.current_game].params["player"]]} ',bg='white',font=tkf.Font(family="Garamond",size=MESSAGE_FONT),fg='#475c49').pack(fill='both',side=tk.RIGHT,expand=True,ipady=MESSAGE_FONT//6)
        frame=tk.Frame(info_window,bd=0,bg='pink3')
        frame.pack(fill='both',side=tk.TOP,expand=True)
        Label(frame,anchor='w',text=f' ПОЧИНАЄ ГРАВЕЦЬ:',bg='#475c49',fg='white').pack(fill='both',side=tk.LEFT,expand=True,ipady=MESSAGE_FONT//6)
        Label(frame,anchor='e',text=f'{player_interpreter[self.games[0].params["turn"]]} ',bg='#475c49',fg='white').pack(fill='both',side=tk.RIGHT,expand=True,ipady=MESSAGE_FONT//6)
        frame=tk.Frame(info_window,bd=0,bg='white')
        frame.pack(fill='both',side=tk.TOP,expand=True)
        Label(frame, anchor='w', text=f'КОЗИРНА МАСТЬ:', bg='white', fg='#475c49').pack(fill='both', side=tk.LEFT, expand=True, ipady=MESSAGE_FONT // 6)
        # Label(frame,anchor='w',text=f' TRUMP SUIT:',bg='white',fg='pink3').pack(fill='both',side=tk.LEFT,expand=True,ipady=MESSAGE_FONT//6)
        Label(frame,anchor='e',text=f'{suits_interpreter[self.games[self.current_game].params["trumps"]]} ',bg='white',fg='#475c49').pack(fill='both',side=tk.RIGHT,expand=True,ipady=MESSAGE_FONT//6)
    ...
    def action(self,**kwargs):
        if self.phase=='deal':
            ...
            while not solver.solved:
                time.sleep(0.005)
                self.text_visited_variable.set(f'РОЗВ\'ЯЗАНО ПІДЗАДАЧ: {len(solver.dp):,}; ЧАС: {time.time()-start_time:.1f} SEC')
                self.root.update()
    ...
    def draw(self):
        ...
        if self.phase=='deal':
            self.draw_players()
            if len(self.games[self.current_game].hands[SOUTH].cards)<10:
                message=f'ВИБРАТИ {10-len(self.games[self.current_game].hands[SOUTH].cards)} КАРТОК ДЛЯ ВАС'
            elif len(self.games[self.current_game].hands[WEST].cards)<10:
                message=f'ВИБРАТИ {10-len(self.games[self.current_game].hands[WEST].cards)} КАРТОК ДЛЯ ГРАВЦЯ АНТОН'
            elif len(self.games[self.current_game].hands[EAST].cards)<10:
                message=f'ВИБРАТИ {10-len(self.games[self.current_game].hands[EAST].cards)} КАРТОК ДЛЯ ГРАВЦЯ ЮЛЯ'
            ...
            # RANDOM DEAL
            button=Button(self.root,text='РАНДОМНА РОЗДАЧА',fg=BACKGROUND_COLOR, bg='white',pady=0,padx=0,command=self.random_deal)


            button.place(relx=0.47,rely=0.37+1.1*CARD_Y_SIZE/Y_SIZE+0.05,anchor='center')

            # UNDO
            button=Button(self.root,text='НАЗАД',fg=BACKGROUND_COLOR,bg='white',pady=0,padx=0,command=self.undo_action)

            button.place(relx=0.30,rely=0.37+1.1*CARD_Y_SIZE/Y_SIZE+0.05,anchor='center')
            if self.current_game==0:
                button['state']='disabled'

            # REDO
            button=Button(self.root,text='ПОВТОРИТИ',fg=BACKGROUND_COLOR,bg='white',pady=0,padx=0,command=self.redo_action)
            button.place(relx=0.67,rely=0.37+1.1*CARD_Y_SIZE/Y_SIZE+0.05,anchor='center')
        ...
        if self.phase=='setup':
            ...
            #CONTRACT TYPE:
            Label(container,text="ТИП КОНТАКТУ:",bg=BACKGROUND_COLOR,fg='white').grid(row=0,columnspan=6,sticky=tk.W)
            button=Button(container,bg='white',text="ГРАТИ",fg=BACKGROUND_COLOR,command=partial(self.action,param='type',val='P'))
            ...
            # PLAYING HAND:
            Label(container,text="ГРАЮЧИЙ РУКА:",bg=BACKGROUND_COLOR,fg='white').grid(row=2,columnspan=6,sticky=tk.W)
            button=Button(container,bg='white',text="ВИ",fg=BACKGROUND_COLOR,command=partial(self.action,param='player',val=0))
            button.grid(row=3,column=0,columnspan=2,sticky=tk.NSEW)
            if self.games[self.current_game].params['player']==SOUTH:
                button.config(bg='#78b1e8',fg='white')
            button=Button(container,bg='white',text="АНТОН",fg=BACKGROUND_COLOR,command=partial(self.action,param='player',val=1))
            button.grid(row=3,column=2,columnspan=2,sticky=tk.NSEW)
            if self.games[self.current_game].params['player']==WEST:
                button.config(bg='#78b1e8',fg='white')
            button=Button(container,bg='white',text="ЮЛЯ",fg=BACKGROUND_COLOR,command=partial(self.action,param='player',val=2))
            button.grid(row=3,column=4,columnspan=2,sticky=tk.NSEW)
            if self.games[self.current_game].params['player']==EAST:
                button.config(bg='#78b1e8',fg='white')

            #FIRST HAND
            Label(container,text="ПОЧИНАЄ ГРАВЕЦЬ:",bg=BACKGROUND_COLOR,fg='white').grid(row=4,columnspan=6,sticky=tk.W)
            button=Button(container,bg='white',text="ВИ",fg=BACKGROUND_COLOR,command=partial(self.action,param='turn',val=0))
            button.grid(row=5,columnspan=2,column=0,sticky=tk.NSEW)
            if self.games[self.current_game].params['turn']==SOUTH:
                button.config(bg='#78b1e8',fg='white')
            button=Button(container,bg='white',text="АНТОН",fg=BACKGROUND_COLOR,command=partial(self.action,param='turn',val=1))
            button.grid(row=5,columnspan=2,column=2,sticky=tk.NSEW)
            if self.games[self.current_game].params['turn']==WEST:
                button.config(bg='#78b1e8',fg='white')
            button=Button(container,bg='white',text="ЮЛЯ",fg=BACKGROUND_COLOR,command=partial(self.action,param='turn',val=2))
            button.grid(row=5,columnspan=2,column=4,sticky=tk.NSEW)
            if self.games[self.current_game].params['turn']==EAST:
                button.config(bg='#78b1e8',fg='white')

            Label(container, text="КОЗИРНА МАСТЬ:", bg=BACKGROUND_COLOR, fg='white').grid(row=6, columnspan=6, sticky=tk.W)
            ...
            if self.games[self.current_game].params['type']=='M':
                button["state"]='disabled'
            button=Button(container,bg='white',text='НЕМАЄ',fg=BACKGROUND_COLOR,command=partial(self.action,param='trumps',val='-'))
            ...
            if self.solution is None:
                ...
                # SUBGAMES SOLVED
                Label(self.root,text=f'РОЗВ\'ЯЗАНО ПІДЗАДАЧ: {self.solution_stats["subgames"]:,}; ЧАС: {self.solution_stats["time"]:.1f} SEC',bg=BACKGROUND_COLOR,fg='white').place(relx=0.5,rely=0.15,anchor='center')
                Button(self.root,text='ПОЧАТИ',bg='white',fg=BACKGROUND_COLOR,command=self.transition_phase).place(relx=0.5,rely=0.21,anchor='center')
        elif self.phase=='play':
            self.draw_players()
            ...
            #current / projected:
            canvas.create_text(CARD_STEP_Y*9//2+CARD_STEP_Y//2,CARD_STEP_X//1.3,angle=0,text=f"поточний/прогнозований: {current_tricks}/{current_tricks+total_tricks}",fill='#78b1e8' if self.games[self.current_game].hands[SOUTH].highlight else 'white',font=tkf.Font(family="Garamond",size=MESSAGE_FONT),anchor='s',justify=tk.CENTER)
            frame=tk.Frame(self.root,bg=BACKGROUND_COLOR,highlightbackground='white',highlightthickness=BORDER_THICKNESS,height=CARD_Y_SIZE+2*BORDER_THICKNESS,width=CARD_X_SIZE+2*BORDER_THICKNESS)
            ...
            canvas.create_text(CARD_STEP_X//1.3,CARD_STEP_Y*9//2+CARD_STEP_Y//2,angle=90,text=f"поточний/прогнозований: {current_tricks}/{current_tricks+total_tricks}",fill='#78b1e8' if self.games[self.current_game].hands[WEST].highlight else 'white',font=tkf.Font(family="Garamond",size=MESSAGE_FONT),anchor='s',justify=tk.CENTER)
            frame=tk.Frame(self.root,bg=BACKGROUND_COLOR,highlightbackground='white',highlightthickness=BORDER_THICKNESS,height=CARD_Y_SIZE+2*BORDER_THICKNESS,width=CARD_X_SIZE+2*BORDER_THICKNESS)
            frame.place(relx=0.5+CARD_X_SIZE/(2*X_SIZE)+3*BORDER_THICKNESS/X_SIZE,rely=0.45-CARD_Y_SIZE/Y_SIZE-6*BORDER_THICKNESS/Y_SIZE,anchor='center')
            ...
            canvas.create_text(CARD_STEP_X,CARD_STEP_Y*9//2+CARD_STEP_Y//2,angle=90,text=f"поточний/прогнозований: {current_tricks}/{current_tricks+total_tricks}",fill='#78b1e8' if self.games[self.current_game].hands[EAST].highlight else 'white',font=tkf.Font(family="Garamond",size=MESSAGE_FONT),anchor='s',justify=tk.CENTER)
            button=Button(self.root,text='НАЗАД',bg='white',fg=BACKGROUND_COLOR,command=self.undo_action)
            button.place(relx=0.34,rely=0.6,anchor='center')
            if self.current_game==0:
                button["state"]='disabled'
            button=Button(self.root,text='ПОВТОРИТИ',bg='white',fg=BACKGROUND_COLOR,command=self.redo_action)
            button.place(relx=0.65,rely=0.6,anchor='center')
            if self.current_game==len(self.games)-1:
                button["state"]='disabled'
            if len(self.games[self.current_game].hands[TRICK].cards)==3:
                Button(self.root,text='ПРОДОВЖИТИ',bg='white',fg=BACKGROUND_COLOR,command=self.flush).place(relx=0.48,rely=0.6,anchor='center')
            ...
            frame=tk.Frame(self.root,bg=BACKGROUND_COLOR,height=Y_SIZE//2,width=X_SIZE//1.1)
            frame.place(relx=0.5,rely=0.5,anchor='center')
            container=Label(frame,bg=BACKGROUND_COLOR)
            container.place(x=0,y=0,height=Y_SIZE//1.95,width=X_SIZE//1.1)
            container.grid_propagate(0)
            container.grid_columnconfigure(0,weight=2,uniform='_')
            for col in range(1,4):
                container.grid_columnconfigure(col,weight=1,uniform='_')
            Label(container,text="ВИ",bg=BACKGROUND_COLOR,fg='#78b1e8' if self.games[0].params['player']==SOUTH else 'white').grid(row=0,column=1,sticky=tk.NSEW,pady=MESSAGE_FONT//3)
            Label(container,text="АНТОН",bg=BACKGROUND_COLOR,fg='#78b1e8' if self.games[0].params['player']==WEST else 'white').grid(row=0,column=2,sticky=tk.NSEW,pady=MESSAGE_FONT//3)
            Label(container,text="ЮЛЯ",bg=BACKGROUND_COLOR,fg='#78b1e8' if self.games[0].params['player']==EAST else 'white').grid(row=0,column=3,sticky=tk.NSEW,pady=MESSAGE_FONT//3)
            Label(container,text="ФІНАЛЬНІ К-СТЬ ТРЮКІВ",bg=BACKGROUND_COLOR,fg='white').grid(row=1,column=0, sticky=tk.W,pady=MESSAGE_FONT//3)
            Label(container,text="ОПТИМАЛЬНА К-СТЬ ТРЮКІВ",bg=BACKGROUND_COLOR,fg='white').grid(row=2,column=0,sticky=tk.W,pady=MESSAGE_FONT//3)
